...
- Ne jamais importer `supabase-js` hors de `src/realtime` — toutes les routes passent par cette abstraction
- Ne pas installer de dépendance sans demander

## Organisation

- Logique partagée (types, machine à états, priorité overlays) → `src/shared` uniquement
...
- Cible 80 % de couverture sur `src/shared` (machine à états, priorités) — logique critique
- Sondage = résultats temps réel pendant le vote ; versus = résultats à la clôture uniquement — comportement à tester dans `src/shared`
